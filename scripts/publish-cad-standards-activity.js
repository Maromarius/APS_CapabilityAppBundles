#!/usr/bin/env node
// Custom activity publisher for CheckCADStandards.
// Differs from publish-autocad-activity.js in that this activity has THREE
// optional get-args (inputFile, paramsFile, standardsFile) and THREE put-args
// (reportJson, summaryCsv, remediationMd) plus result.json for the smoke gate.
...
function buildActivityDef(engineId) {
  return {
    ...
    parameters: {
      // Required: the DWG to check
      inputFile: {
        verb:        'get',
        description: 'Input DWG file to check',
        required:    true,
        localName:   '$(inputFile)',
      },
      // Optional: custom rules JSON payload — localName "params.json" so plugin finds it with File.Exists
      paramsFile: {
        verb:        'get',
        description: 'Custom rules payload JSON',
        required:    false,
        localName:   'params.json',
      },
      // Optional: AutoCAD .dws standards definition file — localName "standards.dws"
      standardsFile: {
        verb:        'get',
        description: 'AutoCAD standards file (.dws)',
        required:    false,
        localName:   'standards.dws',
      },
      // Smoke gate / primary result
      resultJson: {
        verb:        'put',
        description: 'Smoke gate: {ok, overallStatus, pass, warning, fail}',
        required:    false,
        localName:   'result.json',
      },
      // Full per-rule compliance report
      reportJson: {
        verb:        'put',
        description: 'Full compliance report (per-file, per-rule)',
        required:    false,
        localName:   'report.json',
      },
      // One-row CSV for PM dashboards
      summaryCsv: {
        verb:        'put',
        description: 'Summary CSV (one row per file with status counts)',
        required:    false,
        localName:   'summary.csv',
      },
      // Plain-language fix guidance
      remediationMd: {
        verb:        'put',
        description: 'Remediation guidance in Markdown',
        required:    false,
        localName:   'remediation.md',
      },
    },
    description: 'Validates a DWG against CAD standards (layers, text styles, dim styles, linetypes, blocks, purge). Produces report.json, summary.csv, remediation.md.',
  };
}
...
(async () => {
  console.log('══════════════════════════════════════════════════════');
  console.log(` APS Activity Publisher — CheckCADStandards`);
  console.log('══════════════════════════════════════════════════════');
  ...
  console.log(`   Args (get):  inputFile, paramsFile (opt), standardsFile (opt)`);
  console.log(`   Args (put):  resultJson, reportJson, summaryCsv, remediationMd`);
  console.log('══════════════════════════════════════════════════════');
})().catch(err => {
